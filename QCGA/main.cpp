...
void Rotor1XZ()
{
	...
	std::cout << "      C: " << C << std::endl;
	QCGA rot1 = (R1 * c_euc * ~R1) + ((R3 ^ R5) * c_35 * (~R5 ^ ~R3)) + c_2 + ((R2 ^ R4) * c_46 * (~R4 ^ ~R2));
...
void Rotor1YZ()
{
	...
	QCGA taylor_r = r1.rotorExponential(40, phi);
	QCGA taylor_r45 = (r4+r5).rotorExponential(40, phi);
	QCGA taylor_r2367 = (r2+r3+r6+r7).rotorExponential(40, phi);
	std::cout << "      C: " << C << std::endl;
	QCGA rot1 = (taylor_r * c_euc * ~taylor_r);
	QCGA rot2 = (taylor_r45 * c_45 * ~taylor_r45);
	QCGA rot3 = (taylor_r2367 * (3*ei2-2*ei3+6*ei6) * ~taylor_r2367);
